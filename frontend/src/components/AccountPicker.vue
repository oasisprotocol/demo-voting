<script setup lang="ts">
import {computed, ref} from 'vue';

import {Network, networkName, useEthereumStore} from '../stores/ethereum';
import JazzIcon from './JazzIcon.vue';
import AppButton from '@/components/AppButton.vue';
import {abbrAddr} from "@/utils/utils";
import {ICON_SIZE} from "@/constants/constants";

const eth = useEthereumStore();

const netName = computed(() => networkName(eth.network));
const unkNet = computed(() => eth.network === Network.Unknown);

const connecting = ref(false);
const showingConnecting = ref(false);

async function connectWallet() {
  if (connecting.value) return;
  connecting.value = true;
  try {
    setTimeout(() => {
      showingConnecting.value = connecting.value;
    }, 300);
    await eth.connect();
  } finally {
    connecting.value = false;
  }
}
</script>

<template>
  <AppButton
    variant="primary"
    :class="{ 'cursor-default': !!eth.address }"
    class="v-align"
    @click="connectWallet"
  >
    <span class="v-align" v-if="!connecting && eth.address">
      <JazzIcon :size="ICON_SIZE" :address="eth.address"/>
      <span class="text-xs leading-none ml-1">
        <abbr :title="eth.address" class="font-mono block no-underline">{{ abbrAddr(eth.address) }}</abbr>
        <span class="text-2xs" :class="{ 'unk-net': unkNet }">{{ netName }}</span>
      </span>
    </span>
    <span class="v-align" v-else>
      <span class="empty-icon"></span>
      <span class="ml-1 text-sm">
        <span v-if="showingConnecting">Connecting…</span>
        <span v-else>Connect Wallet</span>
      </span>
    </span>
  </AppButton>
</template>

<style lang="postcss" scoped>
.v-align {
  @apply inline-flex items-center;
}

.empty-icon {
  --icon-size: 30px;
  @apply border border-gray-900;

  width: var(--icon-size);
  height: var(--icon-size);
  border-radius: 100%;
  display: inline-block;
}

.empty-icon::after {
  position: relative;
  line-height: var(--icon-size);
  content: '👋';
}

.unk-net {
  @apply text-red-500 underline decoration-wavy decoration-1;
}
</style>
